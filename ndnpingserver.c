...
//Interest name should be ndnx:/name/prefix/ping/number
        ...
        const unsigned char *interest_msg, const struct ndn_parsed_interest *pi)
{
    ...
    if (pi->prefix_comps == prefix_ncomps + 1) {
        number = strtol((char *)interest_msg + pi->offset[NDN_PI_B_LastPrefixComponent] + 2, &end, 10);
        if (*end == '\0')
            return 1;
    }
        ...
        enum ndn_upcall_kind kind, struct ndn_upcall_info *info)
{
    ...
    switch (kind) {
        ...
        case NDN_UPCALL_INTEREST:
            //check Interest name (ndnx:/name/prefix/ping/number)
            if (ping_interest_valid(server->prefix, info->interest_ndnb, info->pi)) {
                ...
                construct_ping_response(info->h, data, info->interest_ndnb, info->pi, server->expire);
